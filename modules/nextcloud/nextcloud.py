...
class NextcloudClient:
    ...
    def _download_file_bytes(self, remote_path: str) -> bytes | None:
        """Download raw file bytes for document workflows."""
        normalized_path = self._normalize_remote_path(remote_path)
        response = self._request(
            "GET",
            self._get_full_url(normalized_path),
    ...
    def _decode_text_bytes(self, content: bytes) -> str:
        """Decode text bytes with a short fallback chain."""
    ...
    def _normalize_analysis_text(self, text: str) -> str:
        """Normalize extracted text for lightweight workflow analysis."""
    ...
    def _extract_readable_text(self, remote_path: str, max_chars: int = 12000) -> dict[str, Any] | None:
        ...
        if extension in TEXT_FILE_EXTENSIONS:
            ...
            print(f"Error extract-actions: unsupported file type '{extension or 'unknown'}'")
            return None

        normalized_text = self._normalize_analysis_text(raw_text)
        if not normalized_text:
            print(f"Error extract-actions: no readable text extracted from '{normalized_path}'")
            return None
    ...
    def extract_actions(self, remote_path: str, max_chars: int = 12000) -> dict[str, Any] | None:
        ...
        extracted = self._extract_readable_text(remote_path, max_chars=max_chars)
...
def run_cli(argv: list[str] | None = None) -> int:
    ...
    if command == "create-tasks-from-file":
        if not command_args:
            print("Usage: nextcloud.py create-tasks-from-file <remote_path> [--mailbox EMAIL] [--priority low|normal|high] [--dry-run]")
            return 1
        ...
        while index < len(command_args):
            ...
            return 1
        result = client.create_tasks_from_file(remote_path, mailbox=mailbox, priority=priority, dry_run=dry_run)
        if result is None:
            return 3
        print_json(result)
        return 0
