...
class Logger:
    ...
    def __init__(self, name: str = "imm-romania"):
        ...
        self.console_enabled = os.environ.get("IMM_LOG_CONSOLE", "true").lower() == "true"
    ...
    def log_response(self, method: str, url: str, status: int, duration_ms: float):
        """Log HTTP response."""
        self.debug(f"Response: {method} {url}", {
            "status": status,
            "duration_ms": duration_ms
        })
    
    def log_email_action(self, action: str, email_id: str, details: Optional[Dict] = None):
        """Log email operation."""
        self.info(f"Email {action}", {"email_id": email_id, **(details or {})})
    
    def log_calendar_action(self, action: str, event_id: str, details: Optional[Dict] = None):
        """Log calendar operation."""
        self.info(f"Calendar {action}", {"event_id": event_id, **(details or {})})
    
    def log_task_action(self, action: str, task_id: str, details: Optional[Dict] = None):
        """Log task operation."""
