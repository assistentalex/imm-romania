...
class TestContractManager(unittest.TestCase):
    ...
    def test_generate_reminder_email_without_renewals(self):
        """Test reminder email when nothing requires renewal."""
        email = self.manager.generate_reminder_email(recipient="test@example.com")

        self.assertFalse(email["has_renewals"])
        self.assertIn("All Contracts Valid", email["subject"])

    def test_get_reminder_history_limit(self):
        """Test reminder history respects limit."""
        client_db = ClientDB(db_path=self.db_path)
        client_db.add_client(name="Test 1", contract_status="renewal_required")
        self.manager.generate_reminder_email(recipient="test@example.com")
        self.manager.generate_reminder_email(recipient="test@example.com")

        history = self.manager.get_reminder_history(limit=1)
        self.assertEqual(len(history), 1)

    def test_mark_client_contacted(self):
        """Test contacted-note helper."""
        client_db = ClientDB(db_path=self.db_path)
        client = client_db.add_client(name="Contacted Client")

        updated = self.manager.mark_client_contacted(client["id"], "Called today")
        self.assertIsNotNone(updated)
        self.assertTrue(any("[CONTACTED] Called today" in note["text"] for note in updated["notes"]))
...
class TestRenewalReminder(unittest.TestCase):
    ...
    def test_add_client_for_renewal_missing_client(self):
        """Test renewal add failure for unknown client."""
        result = self.reminder.add_client_for_renewal(client_id="missing")
        self.assertFalse(result["ok"])
        self.assertIn("not found", result["error"])

    def test_resolve_renewal_missing_client(self):
        """Test renewal resolve behavior for unknown client."""
        result = self.reminder.resolve_renewal(client_id="missing")
        self.assertTrue(result["ok"])
        self.assertIn("not found", result["error"])

    @patch.dict(os.environ, {"MSP_REMINDER_EMAIL": "ops@example.com"}, clear=False)
    def test_run_daily_check_missing_exchange_dependency_records_error(self):
        """Test email path records an error when Exchange send helper is unavailable."""
        client_db = ClientDB(db_path=self.db_path)
        client_db.add_client(name="Renewal Client", contract_status="renewal_required")

        result = self.reminder.run_daily_check(send_email=True, recipient=None)
        self.assertEqual(result["clients_needing_renewal"], 1)
        self.assertFalse(result["email_sent"])
        self.assertIn("error", result)

    def test_get_status_without_history(self):
        """Test status response when no reminder history exists yet."""
        status = self.reminder.get_status()
        self.assertEqual(status["clients_needing_renewal"], 0)
        self.assertEqual(status["recent_reminders"], 0)
        self.assertIsNone(status["last_reminder"])
